...
class Experiment:
    def __init__(self,
        ...
        humidity = pd.read_csv('../../k_submodular/sensor_placement_ts/hums.tsv', sep='\t')
        temp = pd.read_csv('../../k_submodular/sensor_placement_ts/temps.tsv', sep='\t')
        light = pd.read_csv('../../k_submodular/sensor_placement_ts/lights.tsv', sep='\t')
    ...
    def value_function(self, seed_set, reevaluate=False):
        cols = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alg', action='store', type=str, default=None,
                        choices=[
                            'KGreedyTotalSizeConstrained',
                            'KStochasticGreedyTotalSizeConstrained',
                            'ThresholdGreedyTotalSizeConstrained'
                        ])
    ...
    if mode == 'run':
        ...
        plt.savefig(f'{output_dir}/TS-entropy.png', dpi=300, bbox_inches='tight')
        plt.show()
        plt.figure()
        ...
        plt.savefig(f'{output_dir}/TS-eval.png', dpi=300, bbox_inches='tight')
        plt.show()
        plt.figure()
